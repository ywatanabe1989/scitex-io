...
class TestParseSections:
    ...
    def test_recognises_imrad_headers(self):
        ...
        for s in (
            ...
            assert s in out, f"missing section {s}: {list(out)}"
        assert "method body" in out["methods"]

    def test_long_header_not_treated_as_header(self):
        ...
        # No header matched → everything stays in frontpage
        assert "frontpage" in out
        assert "methodology approach" in out["frontpage"]

    def test_summary_and_background_aliases(self):
...
class TestExtractSections:
    def test_clean_pass(self, tmp_path):
        ...
        # At least one IMRaD section must have been parsed
        assert any(s in out for s in ("abstract", "introduction", "methods"))

    def test_no_clean(self, pdf_simple):
...
class TestExtractMetadata:
    def test_basic_fields(self, pdf_simple):
    ...
    def test_custom_metadata_fields(self, pdf_with_meta):
    ...
    def test_unknown_backend_still_returns_core_fields(self, pdf_simple):
        out = _extract_metadata(pdf_simple, "unknown")
        assert out["backend"] == "unknown"
        assert "md5_hash" in out
        # No pages/title because no library branch was taken
        assert "pages" not in out

    def test_fitz_backend(self, pdf_with_meta):
    ...
    def test_pdfplumber_backend(self, pdf_with_meta):
...
class TestExtractMetadataPypdf2Direct:
    def test_no_metadata_object(self, tmp_path):
...
class TestExtractMetadataFitzDirect:
    def test_fitz_populates(self, pdf_with_meta):
        meta = {}
...
class TestExtractMetadataPdfplumberDirect:
    def test_pdfplumber_populates(self, pdf_with_meta):
        meta = {}
...
class TestExtractTables:
    def test_real_extraction(self, pdf_with_table):
...
class TestSaveImage:
    ...
    def test_save_as_jpg_converting_png(self, tmp_path):
        info = {}
        _save_image(
            self._png_bytes(),
            original_ext="png",
    ...
    def test_save_as_jpg_pass_through_for_jpeg(self, tmp_path):
        info = {}
        _save_image(
            self._jpg_bytes(),
            original_ext="jpeg",
        ...
        # original was jpeg, source falls through to the else branch and
        # renames jpeg→jpg
        assert info["ext"] == "jpg"
        assert info["filename"] == "page_2_img_2.jpg"
        assert Path(info["filepath"]).exists()

    def test_save_without_jpg_conversion(self, tmp_path):
        info = {}
        _save_image(
            self._png_bytes(),
            original_ext="png",
    ...
    def test_save_png_already_jpg_extension(self, tmp_path):
    ...
    def test_save_image_pil_unavailable_falls_back(self, tmp_path, monkeypatch):
