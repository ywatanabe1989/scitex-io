...
def test_load_zarr_directory_store(tmp_path):
    p = tmp_path / "a.zarr"
...
def test_load_zarr_zip_store(tmp_path):
    ...
    out = _load_zarr(str(p))
    np.testing.assert_array_equal(out["x"], [10, 20, 30])
...
def test_load_zarr_with_key_navigates_to_subgroup(tmp_path):
    ...
    np.testing.assert_allclose(out["signal"], [1.0, 2.0, 3.0, 4.0])
...
def test_load_zarr_nested_group_recurses(tmp_path):
    p = tmp_path / "e.zarr"
    root = zarr.open(str(p), mode="w")
