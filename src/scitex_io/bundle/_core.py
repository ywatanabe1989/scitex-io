...
def validate_spec(
    ...
    if bundle_type == BundleType.FIGURE:
        from scitex_io.bundle.kinds._figure import validate_figure_spec
...
def load(path: Union[str, Path], in_memory: bool = True) -> Dict[str, Any]:
    ...
    if bundle_type == BundleType.FIGURE:
        from scitex_io.bundle.kinds._figure import load_figure_bundle

        result.update(load_figure_bundle(bundle_dir))
    elif bundle_type == BundleType.PLOT:
        from scitex_io.bundle.kinds._plot._legacy import load_plot_bundle
...
def save(
    ...
    if bundle_type == BundleType.FIGURE:
        from scitex_io.bundle.kinds._figure import save_figure_bundle

        save_figure_bundle(data, dir_path)
    elif bundle_type == BundleType.PLOT:
        from scitex_io.bundle.kinds._plot._legacy import save_plot_bundle
